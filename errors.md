...
| WRN-218 | WRN_LISTENER_SUBSCRIPTION_INVALID | The subscription lookup returned an invalid level, so nearby chatter and scanners remained disabled. | Verify API connectivity and the Radio community subscription, then restart or wait for the automatic refresh. |
