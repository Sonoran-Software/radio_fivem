...
local configFiles = {
    { path = 'earpieces.json', fallback = 'earpieces.DEFAULT.json' },
    { path = 'jammers.json', fallback = 'jammers.DEFAULT.json' },
    { path = 'scanners.json', fallback = 'scanners.DEFAULT.json' },
    { path = 'speakers.json', fallback = 'speakers.DEFAULT.json' },
    { path = 'towers.json', fallback = 'towers.DEFAULT.json' },
    { path = 'mobileRepeaters.json', fallback = 'mobileRepeaters.DEFAULT.json' },
}
local dependencies = { 'qb-core', 'qbx_core', 'es_extended', 'ox_lib', 'ox_inventory', 'qb-inventory', 'ps-inventory', 'qs-inventory', 'lb-phone', 'sonorancad' }
...
local function collectSupportLogs()
    ...
    local prefix = ([[SonoranRadio Support Output
---------------------------------------
Runtime Snapshot
----------------
%s
...
function UploadRadioSupportLogs(ticketId)
    local id = tonumber(ticketId)
    if not id or id < 1 or id > 2147483647 or id % 1 ~= 0 then
        errorLog('ERR_SUPPORT_INVALID_ID', 'Use sonoranradio support <ticket ID> with the number provided by support.')
        return
    end
    ...
    if not ok then
        errorLog('ERR_SUPPORT_COLLECT_FAILED', 'Unable to collect support diagnostics. Check the resource configuration and restart it.')
        return
    end
    if #body > 1000000 then
        errorLog('ERR_SUPPORT_TOO_LARGE', 'Support output exceeds the 1 MB upload limit. Contact support for another way to send the log.')
        return
    end
    uploadPending = true
    ...
    local function finish(status, response)
        ...
        if tonumber(status) == 200 and decodedOk and type(result) == 'table' and result.success == true then
            ...
            errorLog('ERR_SUPPORT_UPLOAD_FAILED', ('Support upload failed (HTTP %s). Verify the ticket ID and ask support to enable debug uploads, then retry.'):format(tostring(status)))
        end
    end
    SetTimeout(30000, function() finish('timeout', '') end)
    local requested = pcall(PerformHttpRequest,
        ('https://api.sonoransoftware.com/v2/upload/debug/%d?product=radio'):format(id),
