        ...
        req.setDataHandler(function(data)
            ...
            if body.key and body.key:upper() == Config.apiKey:upper() then
                if plugin_handlers[body.type] ~= nil then
                    ...
                    TriggerEvent('SonoranRadio::core:writeLog', 'warn', 'Received push event for unregistered type: ' .. tostring(body.type))
                    res.send('Event not registered')
                end
            else
                TriggerEvent('SonoranRadio::core:writeLog', 'warn', 'Received push event with invalid API key.')
